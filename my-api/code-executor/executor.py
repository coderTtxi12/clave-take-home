...
This FastAPI service runs in a separate Docker container to provide secure,
isolated execution of Python code generated by the coding agent.

Key Features:
- Isolated execution environment (separate container)
- Read-only database access (via db_helper)
- Access to data files (mounted volumes)
- Captures stdout/stderr for results
- Timeout protection (via Docker resource limits)
- No network access (except to database)

Security:
- Code runs in isolated container (cannot affect main API)
- Database connection is read-only (prevents data modification)
- Resource limits prevent abuse (CPU, memory)
