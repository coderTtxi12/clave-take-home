...
from prompts.prompts import SYSTEM_PROMPT_DATA_ANALYST, SYSTEM_PROMPT_WEB_DEV
...
@router.post(
    "/fullstack-coding-agent/query",
    response_model=CodingAgentResponse,
    status_code=status.HTTP_200_OK,
    summary="Fullstack coding agent",
    description="Fullstack coding agent endpoint for web development tasks"
)
async def fullstack_coding_agent_endpoint(request: CodingAgentRequest):
    """
    Fullstack coding agent endpoint - natural language to web development
    ...
    Returns:
        CodingAgentResponse: Answer, code executed, results, and conversation history
    """
    logger.info(f"Fullstack coding agent query: {request.query[:100]}...")
    ...
    try:
        ...
        # Use all tools for fullstack development
        tools = all_tools
        tools_schemas = all_tools_schemas
        ...
        logger.error(f"Fullstack coding agent error: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Fullstack coding agent failed: {str(e)}"
        )
